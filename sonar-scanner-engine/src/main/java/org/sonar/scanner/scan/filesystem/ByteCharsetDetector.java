...
public class ByteCharsetDetector {
  ...
  public Charset detect(byte[] buf) {
    ...
    if (utf8Result.valid() == Valid.YES) {
      return utf8Result.charset();
    } else if (utf8Result.valid() == Valid.MAYBE) {
    ...
    if (utf16.valid() == Valid.YES && validator.isValidUTF16(buf, UTF_16LE.equals(utf16.charset()))) {
      return utf16.charset();
    }

    // at this point we know it can't be UTF-8
    for (Charset c : charsets) {
      if (!UTF_8.equals(c) && (!isUtf16(c) || utf16.valid() == Valid.MAYBE) && validator.tryDecode(buf, c)) {
        return c;
      }
    }
  ...
  private Charset detectAscii(byte[] buf, Set<Charset> charsets) {
    ...
    return charsets.contains(UTF_8) ? null : UTF_8;
  }

...
public class CharsetValidation {
  ...
  public Result isUTF16(byte[] buffer, boolean failOnNull) {
    ...
    return new Result(Valid.MAYBE, null);
  }

  public boolean looksBinary() {
    return false;
  }
  ...
  public Result isUTF8(byte[] buffer, boolean rejectNulls) {
    ...
    for (int i = 0; i < buffer.length; i++) {
      ...
      if ((c & 0b10000000) == 0) {
        len = 0;
      } else if ((c & 0b1110000) == 0b11000000) {
        len = 1;
    ...
    return onlyAscii ? new Result(Valid.MAYBE, StandardCharsets.UTF_8) : Result.newValid(StandardCharsets.UTF_8);
  }
  ...
  public static class Result {
    private static Result INVALID = new Result(Valid.NO, null);
    private Valid valid;
    private Charset charset;

    private Result(Valid valid, @Nullable Charset charset) {
      this.valid = valid;
      this.charset = charset;
    }

    public static Result newValid(Charset charset) {
      return new Result(Valid.YES, charset);
    }
